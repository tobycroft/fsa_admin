 ...
 */
class TagDataunit extends Admin
{
    /**
     * 用户首页
     * @return mixed
...
//            'class' => 'btn btn-xs btn-default ajax-get',
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($user = TagDataunitModel::create($data)) {
    ...
    public function edit($id = null)
    {
        ...
        if (session('user_auth.role') != 1) {
            ...
            $user_list = TagDataunitModel::where('role', 'in', $role_list)->column('id');
        ...
        if ($this->request->isPost()) {
            ...
            if (TagDataunitModel::update($data)) {
                $this->success('编辑成功');
            } else {
                $this->error('编辑失败');
            }
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === TagDataunitModel::where('id', 'in', $ids)->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
            case 'disable':
                if (false === TagDataunitModel::where('id', 'in', $ids)->setField('status', 0)) {
                    $this->error('禁用失败');
                }
                break;
            case 'delete':
                if (false === TagDataunitModel::where('id', 'in', $ids)->delete()) {
                    $this->error('删除失败');
                }
                break;
            default:
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = TagDataunitModel::where("id", $id)->setField($field, $value);
        if (false !== $result) {
            $this->success('操作成功');
        } else {
            $this->error('操作失败');
