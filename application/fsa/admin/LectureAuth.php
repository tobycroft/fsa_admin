...
use app\fsa\model\LectureAuthModel;
 ...
 */
class LectureAuth extends Admin
{
    /**
     * 用户首页
     * @return mixed
    ...
    public function index()
    {
        ...
        // 读取用户数据
        $data_list = LectureAuthModel::where($map)->order($order)->paginate();
...
//            'class' => 'btn btn-xs btn-default ajax-get',
        ...
        return ZBuilder::make('table')
            ...
            ->addColumns([
                ["id", "id"],
                ["lid", "讲座ID"],
                ['type', '讲座类型', 'select', ["无" => "none", "讲师" => "instructor", "协会" => "association", "host" => "host", "平台" => "platform"]],
                ["right_button", "功能"],
            ])
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($user = LectureAuthModel::create($data)) {
    ...
    public function edit($id = null)
    {
        ...
        if (session('user_auth.role') != 1) {
            ...
            $user_list = LectureAuthModel::where('role', 'in', $role_list)->column('id');
        ...
        if ($this->request->isPost()) {
            ...
            if (LectureAuthModel::update($data)) {
                $this->success('编辑成功');
            } else {
                $this->error('编辑失败');
            }
        ...
        // 获取数据
        $info = LectureAuthModel::where('id', $id)->find();
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === LectureAuthModel::where('id', 'in', $ids)->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
            case 'disable':
                if (false === LectureAuthModel::where('id', 'in', $ids)->setField('status', 0)) {
                    $this->error('禁用失败');
                }
                break;
            case 'delete':
                if (false === LectureAuthModel::where('id', 'in', $ids)->delete()) {
                    $this->error('删除失败');
                }
                break;
            default:
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = LectureAuthModel::where("id", $id)->setField($field, $value);
        if (false !== $result) {
            $this->success('操作成功');
        } else {
            $this->error('操作失败');
