...
use app\fsa\model\RateThreadReplyModel;
 ...
 */
class RateThreadReply extends Admin
{
    /**
     * 用户首页
     * @return mixed
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = RateThreadReplyModel::where($map)->order($order)->paginate();
        $page = $data_list->render();
//        $todaytime = date('Y-m-d H:i:s', strtotime(date("Y-m-d"), time()));

//        $num1 = RateThreadReplyModel::where("date", ">", $todaytime)->count();
//        $num2 = RateThreadReplyModel::count();
...
//            ->setPageTips("总数量：" . $num2, 'danger')
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($user = RateThreadReplyModel::create($data)) {
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (RateThreadReplyModel::update($data)) {
                $user = RateThreadReplyModel::get($data['id']);
                // 记录行为
        ...
        // 获取数据
        $info = RateThreadReplyModel::where('id', $id)->find();
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === RateThreadReplyModel::where('id', 'in', $ids)->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
            case 'disable':
                if (false === RateThreadReplyModel::where('id', 'in', $ids)->setField('status', 0)) {
                    $this->error('禁用失败');
                }
                break;
            case 'delete':
                if (false === RateThreadReplyModel::where('id', 'in', $ids)->delete()) {
                    $this->error('删除失败');
                }
                break;
            default:
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = RateThreadReplyModel::where("id", $id)->setField($field, $value);
        if (false !== $result) {
            action_log('user_edit', 'user', $id, UID);
            $this->success('操作成功');
        } else {
