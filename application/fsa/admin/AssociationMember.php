 ...
 */
class AssociationMember extends Admin
{
    /**
     * 用户首页
     * @return mixed
    ...
    public function index()
    {
        ...
        foreach ($data_list as $key => $item) {
            $item = AssociationModel::where("id", $item["aid"])->allowField("name")->find();
            $data_list[$key] = $item;
        }
        $btn_access = [
            'title' => '回复',
...
//            'class' => 'btn btn-xs btn-default ajax-get',
    ...
    public function edit($id = null)
    {
        ...
        if (session('user_auth.role') != 1) {
            ...
            $user_list = AssociationMemberModel::where('role', 'in', $role_list)->column('id');
        ...
        if ($this->request->isPost()) {
            ...
            if (AssociationMemberModel::update($data)) {
                $this->success('编辑成功');
            } else {
                $this->error('编辑失败');
            }
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === AssociationMemberModel::where('id', 'in', $ids)->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
            case 'disable':
                if (false === AssociationMemberModel::where('id', 'in', $ids)->setField('status', 0)) {
                    $this->error('禁用失败');
                }
                break;
            case 'delete':
                if (false === AssociationMemberModel::where('id', 'in', $ids)->delete()) {
                    $this->error('删除失败');
                }
                break;
            default:
