...
class TagForm extends Admin
{
    ...
    public function index()
    {
        ...
        // 读取用户数据
        $data_list = TagFormModel::where($map)->order($order)->paginate();
...
//            ->addRightButton("custom", $btn_access)
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($user = TagFormModel::create($data)) {
    ...
    public function edit($id = null)
    {
        ...
        if (session('user_auth.role') != 1) {
            ...
            $user_list = TagFormModel::where('role', 'in', $role_list)->column('id');
        ...
        // 获取数据
        $info = TagFormModel::where('id', $id)->find();
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === TagFormModel::where('id', 'in', $ids)->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
            case 'disable':
                if (false === TagFormModel::where('id', 'in', $ids)->setField('status', 0)) {
                    $this->error('禁用失败');
                }
                break;
            case 'delete':
                if (false === TagFormModel::where('id', 'in', $ids)->delete()) {
                    $this->error('删除失败');
                }
                break;
            default:
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = TagFormModel::where("id", $id)->setField($field, $value);
        if (false !== $result) {
            $this->success('操作成功');
        } else {
            $this->error('操作失败');
