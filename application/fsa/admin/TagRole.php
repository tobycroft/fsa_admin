...
class TagRole extends Admin
{
    ...
    public function index()
    {
        ...
        // 读取用户数据
        $data_list = TagRoleModel::where($map)->order($order)->paginate();
...
//            ->addRightButton("custom", $btn_access)
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($user = TagRoleModel::create($data)) {
    ...
    public function edit($id = null)
    {
        ...
        if (session('user_auth.role') != 1) {
            ...
            $user_list = TagRoleModel::where('role', 'in', $role_list)->column('id');
        ...
        // 获取数据
        $info = TagRoleModel::where('id', $id)->find();
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === TagRoleModel::where('id', 'in', $ids)->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
            case 'disable':
                if (false === TagRoleModel::where('id', 'in', $ids)->setField('status', 0)) {
                    $this->error('禁用失败');
                }
                break;
            case 'delete':
                if (false === TagRoleModel::where('id', 'in', $ids)->delete()) {
                    $this->error('删除失败');
                }
                break;
            default:
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = TagRoleModel::where("id", $id)->setField($field, $value);
        if (false !== $result) {
            $this->success('操作成功');
        } else {
            $this->error('操作失败');
